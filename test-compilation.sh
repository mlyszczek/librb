...
prepare()
{
	...
	# clone
	if ! git clone "$project_dir" "${project}-${slot}"; then
...
build()
{
	...
	if [ ${ret} -eq 0 ]; then
		printf "[ok]  %8d    ${opts}\n" ${seq} >> \
			"${workdir}/compilation-test-results"
...
## ==========================================================================
project="librb"
workdir="/tmp/parallel-test-compilation-$project"
...
# distcheck (when failed) leaves readonly files which cannot be deleted
...
parallel --output-as-files --bar --results "${workdir}" \
	...
	prepare ::: "${project}" ::: ${slots} ::: "${project_dir}" ::: $commit
